...
import { Play, Star, Lock, ChevronRight, Code2, ArrowLeft } from 'lucide-react';
      ...
      <div className="lg:hidden px-4 py-4 min-h-[calc(100vh-73px)]">
        ...
        <div className="rounded-2xl p-5 flex flex-col gap-4"
          ...
          <div className="flex items-center justify-between p-3 rounded-xl"
            ...
            <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Dificultad</span>
          ...
          <div className="flex gap-3">
            {[{ val: mLang.levels.length, lbl: 'Niveles' }, { val: mTotalConcepts, lbl: 'Conceptos' }].map(({ val, lbl }) => (
              <div key={lbl} className="flex-1 rounded-xl p-3 text-center"
                ...
                <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{lbl}</div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="min-h-[calc(100vh-73px)]">
      ...
      <div className="lg:hidden px-4 py-4">
        <div className="mb-4">
          <h2 className="text-lg font-bold" style={{ color: 'var(--foreground)' }}>Elige un lenguaje</h2>
            ...
            return (
              ...
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                  ...
                  <LanguageIcon name={l.icon} size={28} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-base" style={{ color: 'var(--foreground)' }}>{l.name}</span>
      ...
      <div className="hidden lg:flex gap-5 px-6 py-4">

        {/* Panel izquierdo */}
        <div className="w-[340px] flex-shrink-0 flex flex-col gap-4">
          <div>
            <h2 className="text-xl font-bold tracking-tight" style={{ color: 'var(--foreground)' }}>Elige un lenguaje</h2>
            <p className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>Selecciona y empieza a practicar</p>
          </div>

          <div className="flex-1 rounded-[24px] p-6 flex flex-col transition-all duration-500 relative overflow-hidden"
            style={{
            ...
            <div className="absolute -top-20 -right-20 w-56 h-56 rounded-full blur-[60px] opacity-20 pointer-events-none"
              style={{ background: lang.accentColor }} />

            <div className="flex items-center gap-4 mb-6 relative z-10">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0"
                style={{ background: `linear-gradient(135deg, ${lang.accentColor}20, ${lang.accentColor}05)`, border: `1px solid ${lang.accentColor}40` }}>
                <LanguageIcon name={lang.icon} size={40} />
              </div>
              <div>
                <h3 className="font-extrabold text-2xl tracking-tight mb-1.5" style={{ color: 'var(--foreground)' }}>{lang.name}</h3>
            ...
            <div className="flex gap-3 mb-6 relative z-10">
              {[{ val: lang.levels.length, lbl: 'Niveles' }, { val: totalConcepts, lbl: 'Conceptos' }].map(({ val, lbl }) => (
                <div key={lbl} className="flex-1 rounded-xl p-3.5 flex flex-col items-center justify-center"
                  style={{ background: 'rgba(0,0,0,0.1)', border: '1px solid rgba(255,255,255,0.03)' }}>
                  <span className="text-2xl font-black mb-1" style={{ color: 'var(--foreground)' }}>{val}</span>
                  <span className="text-[10px] font-bold tracking-widest uppercase text-muted-foreground">{lbl}</span>
                </div>
              ))}
            </div>

            <div className="rounded-xl overflow-hidden mb-6 shadow-xl relative z-10" style={{ background: '#0d1117', border: '1px solid #30363d' }}>
              <div className="flex items-center gap-2 px-4 py-3" style={{ background: '#161b22', borderBottom: '1px solid #30363d' }}>
                ...
                <span className="text-[11px] ml-2 font-mono text-[#8b949e]">preview.{lang.name.toLowerCase()}</span>
              </div>
              <pre className="p-4 text-[13px] leading-relaxed font-mono overflow-auto" style={{ color: '#e6edf3', maxHeight: '160px' }}>
                {previewCode}
              </pre>
            </div>
        ...
        <div className="flex-1 flex flex-col gap-4 pl-2">
          <div>
            <h2 className="text-lg font-semibold" style={{ color: 'var(--foreground)' }}>Todos los lenguajes</h2>
            <p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>{languages.length} disponibles</p>
          </div>
                ...
                >
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0 pr-4">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold truncate text-base" style={{ color: 'var(--foreground)' }}>{l.name}</span>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 w-full" style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    {l.difficulty === 'Avanzado' && (
                      <div className="flex items-center gap-1.5">
                        ...
                        <span style={{ color: 'var(--chart-1)', fontSize: '11px', fontWeight: 600 }}>Exp. recomendada</span>
                      </div>
                    )}
                  </div>
                </button>
